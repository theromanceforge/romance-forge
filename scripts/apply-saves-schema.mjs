#!/usr/bin/env node
/**
 * Apply public.saves + RLS using SUPABASE_SECRET_KEY from .env.local.
...
const sqlPath = path.join(root, 'scripts/apply-saves-schema.sql');
...
const { error: probeErr } = await admin.from('saves').select('user_id').limit(1);
if (!probeErr) {
  console.log('PROBE: public.saves reachable — schema OK (or already applied)');
  process.exit(0);
}
console.log('PROBE: saves table →', probeErr.message || probeErr.code || 'error');

if (!applied) {
  console.error('');
  console.error('BLOCKER: Could not apply DDL programmatically.');
  console.error('Open Supabase Dashboard → SQL Editor and run:');
  console.error('  scripts/apply-saves-schema.sql');
  process.exit(2);
}
