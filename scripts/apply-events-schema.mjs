#!/usr/bin/env node
/**
 * Apply public.events + RLS (insert-only analytics) using SUPABASE_SECRET_KEY from .env.local.
...
const sqlPath = path.join(root, 'scripts/apply-events-schema.sql');
...
// Preferred when available: Supabase Management API with a personal access token
// (SUPABASE_ACCESS_TOKEN=sbp_… in .env.local). The project secret key alone cannot run DDL.
...
if (pat) {
  ...
  try {
    ...
    console.log('DDL attempt <management-api>/database/query →', res.status);
    if (res.ok) {
      applied = true;
...
for (const ep of applied ? [] : endpoints) {
  try {
...
const { error: probeErr } = await admin.from('events').select('id').limit(1);
if (!probeErr) {
  console.log('PROBE: public.events reachable — schema OK (or already applied)');
  process.exit(0);
}
console.log('PROBE: events table →', probeErr.message || probeErr.code || 'error');

if (!applied) {
  console.error('');
  console.error('BLOCKER: Could not apply DDL programmatically.');
  console.error('Open Supabase Dashboard → SQL Editor and run:');
  console.error('  scripts/apply-events-schema.sql');
  process.exit(2);
}
