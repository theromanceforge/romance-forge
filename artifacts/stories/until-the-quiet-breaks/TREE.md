...
- **→ scene2a** — Find the blue door — seek Jake at the diner
- **→ scene2b** — Walk to Willow Lane — face Henry first

## Layer 2

### scene2a — The Diner (Jake)
- **→ scene3a** — Stay through the dinner rush — talk with Jake alone
- **→ scene3b** — Ask Jake to walk you to Willow — face Henry together

### scene2b — Willow Lane (Henry)
- **→ scene3c** — Press Henry for the whole truth now
- **→ scene3d** — Leave for the diner — hear Jake's side before Henry finishes
...
- **→ scene4c** — Let Jake speak for you when Henry opens the door
- **→ scene4d** — Take the lead — confront Henry yourself

### scene3c — Henry Pressed (from 2b)
- **→ scene4e** — Believe Henry enough to go find Jake with open hands
- **→ scene4f** — Doubt him — search the house with Clara's sketches as a map

### scene3d — Back to the Blue Door (from 2b)
- **→ scene4g** — Tell Jake everything Henry hinted at
- **→ scene4h** — Shield Jake from Henry's version — ask only what he remembers of the night you left

## Layer 4

### scene4a — The Almost
- **→ scene5a** — Pull Clara into the booth — read the papers with Jake present
...
- **→ scene5f** — Take Jake outside — decide what the family will say next

### scene4d — The Indictment
- **→ scene5g** — Force Henry to name the creditor who still shadows the Shaws
- **→ scene5h** — Walk Jake to the diner — let the indictment travel with you

### scene4e — Open Hands
- **→ scene5i** — Kiss Jake with the open secret still unnamed
...
- **→ scene5l** — Bring the sketches to Jake before Henry can reclaim them

### scene4g — The Spill
- **→ scene5m** — Stay and weather Jake's anger until it softens
...
|----|-------|----------|----------|
| `scene5a` | Papers in the Booth | → `scene6a` — Show Jake every line Clara circled | → `scene6b` — Hide the ugliest page — protect him one more hour |
| `scene5b` | Kiss Then Evidence | → `scene6b` — Let Clara narrate what she found | → `scene6c` — Send Clara out — ask Jake what he wants from you tonight |
...
| `scene5d` | Rain First | → `scene6e` — At first light, demand the full cover story | → `scene6f` — At first light, ask Jake to choose you over the Shaw quiet |
...
| `scene5f` | Outside Willow | → `scene6h` — Plan a private family council with Clara | → `scene6i` — Kiss Jake in the wet boxwood — steal one honest hour |
| `scene5g` | Name the Creditor | → `scene6j` — Go with Henry to settle the ghost debt | → `scene6k` — Refuse Henry's money — demand only the truth spoken to Jake |
| `scene5h` | Indictment Travels | → `scene6k` — Tell Clara first so she isn't blindsided | → `scene6l` — Tell the story only to Jake behind the locked blue door |
| `scene5i` | Open-Secret Kiss | → `scene6c` — After, open Henry's letter together | → `scene6g` — After, go to Willow before courage cools |
| `scene5j` | Letter on the Counter | → `scene6a` — Stay while Jake reads — answer every question | → `scene6m` — Give him space; wait at Willow with Clara |
| `scene5k` | Shed and Ledger | → `scene6n` — Open the ledger with Clara before anyone else | → `scene6o` — Bring Jake to the shed — no more secondhand truth |
| `scene5l` | Sketches to Jake | → `scene6o` — Sit with him while he studies Clara's margins | → `scene6p` — Ask him not to forgive Henry yet |
...
| ID | Title | Choice A | Choice B |
|----|-------|----------|----------|
| `scene6a` | Every Circled Line | → `scene7a` — Burn nothing — photocopy and confront Henry at noon | → `scene7b` — Keep the papers but ask Jake what justice looks like |
...
| `scene6d` | Wake Henry | → `scene7f` — Make him confess in front of Jake | → `scene7g` — Make him confess to you alone, then relay |
...
| `scene6k` | Truth Not Money | → `scene7e` — Bring Jake into the refusal | → `scene7g` — Shield Jake; carry the refusal yourself |
| `scene6l` | Locked Blue Door Story | → `scene7c` — When you finish, invite Clara in | → `scene7f` — When you finish, go to Henry before dawn |
| `scene6m` | Wait at Willow | → `scene7n` — Use the wait to search Henry's desk again | → `scene7o` — Use the wait to prepare Clara for Jake's rage |
| `scene6n` | Ledger First | → `scene7n` — Photograph every page for safekeeping | → `scene7a` — Confront Henry with the original in hand |
| `scene6o` | Shed with Jake | → `scene7p` — Let him break what needs breaking | → `scene7e` — Hold him back — breaking won't rewrite the dead |
...
| `scene7b` | Justice His Shape | → `scene8c` — Agree to Jake's terms even if they scare you | → `scene8d` — Negotiate — your life is not only his wound |
| `scene7c` | Clara Second Reading | → `scene8e` — Let Clara decide who hears first | → `scene8c` — Overrule her gently — Jake must hear first |
...
| `scene7f` | Confess Before Jake | → `scene8h` — Hold Jake's hand while Henry speaks | → `scene8i` — Stand apart — let Jake face his uncle alone |
| `scene7g` | Relay Alone | → `scene8g` — Softening nothing when you tell Jake | → `scene8j` — Softening only the parts that would destroy him |
...
| `scene7j` | Secret a Little Longer | → `scene8j` — Set a deadline: one week, then full disclosure | → `scene8c` — Admit you're afraid and ask Jake to decide the deadline |
...
| `scene7n` | Photograph Pages | → `scene8k` — Send copies to a lawyer outside Somerton | → `scene8e` — Give copies only to Clara and Jake |
...
| `scene7p` | Break or Hold | → `scene8i` — Let Jake smash the shed lock; stay with the wreckage | → `scene8d` — Pull him into the rain and choose living over wreckage |

## Layer 8

| ID | Title | Choice A | Choice B |
|----|-------|----------|----------|
| `scene8a` | Recorded Confession | → `scene9a` — Release the recording to Jake and Clara only | → `scene9b` — Hold the recording as insurance — speak live instead |
| `scene8b` | Written Statement | → `scene9a` — Notarize Henry's statement at the county office | → `scene9e` — Burn the statement after Jake reads it — no paper trail |
| `scene8c` | Jake's Terms | → `scene9b` — Accept a life rebuilt in Somerton on his terms | → `scene9c` — Accept love but refuse to erase your years away |
...
| `scene8e` | Clara Chooses Audience | → `scene9d` — Support her choice even if it scares you | → `scene9b` — Ask her to delay for Jake's sake |
...
| `scene8h` | Hand in Hand | → `scene9g` — Forgive Henry in Jake's hearing | → `scene9b` — Refuse forgiveness; keep holding Jake's hand |
| `scene8i` | Jake Alone with Henry | → `scene9e` — Wait outside; trust him to return | → `scene9a` — Interrupt if the silence lasts too long |
...
| `scene8n` | Second Letter / Cost | → `scene9a` — Read Henry's unsent letter to Jake | → `scene9e` — Give Jake the choice to read or not |
...
| ID | Title | Choice A | Choice B |
|----|-------|----------|----------|
| `scene9a` | Insurance and Kin | → `scene10a` — Choose full disclosure with Jake — build a life in daylight | → `scene10i` — Choose a sealed family archive — love with careful silence |
...
| `scene9c` | Honesty Before Spectacle | → `scene10a` — Commit to Jake without erasing your years away | → `scene10j` — Commit to trying — leave a soft door to leave |
...
| `scene9g` | Forgive or Exile | → `scene10f` — Vote with Jake to keep Henry at Willow in grace | → `scene10g` — Vote with Jake to cut Henry out of the diner's future |
| `scene9h` | Station Undecided | → `scene10c` — Board the train alone — free of the lie | → `scene10j` — Pull Jake onto the platform — unfinished promise |

## Layer 10 — endings (`choices: []`)

| ID | Ending | Destination |
|----|--------|-------------|
| `scene10a` | Together in Truth | Jake and [player_name] stay; the secret is spoken; the diner opens in daylight. |
| `scene10b` | Soft Rebuild | Love holds while a partial silence still heals; the quiet is thinner, not gone. |
| `scene10c` | Leave Free | She leaves Somerton unburdened; Jake respects the freedom he once lost. |
| `scene10d` | Stay Cold | She remains in town without Jake — present, untied, unfinished. |
...
Layers 5–10: see tables above. Tracks weave **Truth** (papers/Henry/creditor), **Heart** (Jake intimacy), **Clara** (evidence/agency), and **Reckoning** (town/public vs private).
